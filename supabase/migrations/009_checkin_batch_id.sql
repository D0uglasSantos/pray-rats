-- Vincula check-ins criados juntos em múltiplos grupos (CRUD em lote).
-- Nenhum dado existente é removido.

alter table public.checkins
  add column if not exists batch_id uuid;

create index if not exists idx_checkins_batch_id
  on public.checkins(batch_id)
  where batch_id is not null;

create index if not exists idx_checkins_user_batch
  on public.checkins(user_id, batch_id)
  where batch_id is not null;
...
revoke all on function public.create_checkin_safely(uuid, uuid, text, text, integer, numeric, checkin_visibility, text, uuid) from public;
grant execute on function public.create_checkin_safely(uuid, uuid, text, text, integer, numeric, checkin_visibility, text, uuid) to authenticated;
