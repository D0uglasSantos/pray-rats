-- Permite informar data/hora do check-in na criação.
-- Limites diário/semanal passam a considerar a data informada.
...
declare
  ...
  v_checkin_id uuid;
  v_checked_in_at timestamptz := coalesce(create_checkin_safely.checked_in_at, now());
begin
  ...
  if v_checked_in_at > now() then
    raise exception using message = 'CHECKIN_DATETIME_FUTURE', errcode = 'P0001';
  end if;
  ...
  if v_activity.daily_limit is not null then
    ...
    where c.user_id = v_user_id
      ...
      and c.status = 'valid'
      and c.checked_in_at >= date_trunc('day', v_checked_in_at)
      and c.checked_in_at < date_trunc('day', v_checked_in_at) + interval '1 day';
  ...
  if v_activity.weekly_limit is not null then
    ...
    where c.user_id = v_user_id
      and c.activity_type_id = v_activity.id
      and c.status = 'valid'
      and c.checked_in_at >= date_trunc('week', v_checked_in_at)
      and c.checked_in_at < date_trunc('week', v_checked_in_at) + interval '1 week';
  ...
  values (
    ...
    v_activity.points,
    'valid',
    create_checkin_safely.batch_id,
    v_checked_in_at
  )
...
revoke all on function public.create_checkin_safely(uuid, uuid, text, text, integer, numeric, checkin_visibility, text, uuid, timestamptz) from public;
grant execute on function public.create_checkin_safely(uuid, uuid, text, text, integer, numeric, checkin_visibility, text, uuid, timestamptz) to authenticated;
